...
def upload_file():
    ...
    if file_path:
        model = load_model()  
        process_and_plot_data(model, file_path)
...
def process_and_plot_data(model, file_path):
    try:
...
def display_graph(df_grouped):
    try:
        ...
        fig, ax = plt.subplots()
        ax.plot(df_grouped['Date'], df_grouped['Sales'], label='Прогноз будущих продаж')
        ax.set_title('Прогноз будущих продаж')
        ax.set_xlabel('Дата')
    ...
    except Exception as e:messagebox.showerror("Ошибка", f"Не удалось обработать данные: {e}")
